...
export const initSockets = (httpServer: Server) => {
  ...
  io.use((socket, next) => {
    ...
    if (!token || token === 'dummy-token') {
      // 🛡️ SECURITY: Unique IDs for Guests to prevent Host collisions
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('join_workbook', async (payload: { workbookId: string, name?: string }, callback) => {
      try {
        ...
        // 🛡️ SECURITY: Automatic Host Attribution
        if (!currentHost) {
        ...
        const activeHost = await redis.get(`room:host:${workbookId}`);
        const isHost = activeHost === userId;
    ...
    socket.on('disconnect', async () => {
      if (currentRoom && currentWorkbookId) {
        ...
        // 🛡️ SECURITY: Host Handover Logic
        ...
        if (currentHost === userId) {
          ...
          if (socketsInRoom.length > 0) {
            // Transfer host to the next available user
            ...
            // Room is empty, clear the host and lock
